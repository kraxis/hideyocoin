...
#if defined(NDEBUG)
# error "HideYoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd096159f0072a7c792d0c24a6a619e7aabd4210b6ec8c1e605e017e124f4c65b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HideYoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HideYoCoin Signed Message:\n";
...
{
    // HideYoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // HideYoCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // HideYoCoin: 0.5 days
static const int64 nTargetSpacing = 1 * 60; // HideYoCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HideYoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HideYoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // HideYoCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8942153157e8780ef50ce545d0d51662325646b514a2ad66133835892af30922");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "WIRED 6/13/14 Next Week Is a National Celebration of Plant Sex";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402975551; //new epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085103723;

        if (fTestNet)
        {
            block.nTime    = 1402715920; //epochtime
            block.nNonce   = 385570156;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2432a8acaedd9196498ce9e6bffb9d65cf3e5711d35b78e45556ff821c78c739"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // HideYoCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HideYoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HideYoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HideYoCoinMiner(CWallet *pwallet)
{
    printf("HideYoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hideyocoin-miner");
    ...
    try { loop {
        ...
        printf("Running HideYoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HideYoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HideYoCoinMiner, pwallet));
}
